...
- 实时说话人分离支持**断点续传**（`speaker context`）：
  - **v2 实时接口同步支持**：URL query 参数 `enable_speaker_context` / `speaker_context_id`
    ...
    `SpeechRecognitionResponse.SpeakerContinue`，经 `SpeechRecognizer.SpeakerContinue()` 读取
  ...
  - 首响应新增 `speaker_continue`（`continue_status`: `fresh`/`resumed`/
    ...
    `SpeechRecognizer.SpeakerContinue()` 以及 `OnRecognitionStart` 回调
    （`SpeechRecognitionResponse.SpeakerContinue`）解出。
  ...
  - 两个参数需与 `speaker_diarization=1/3` 同开，否则 `Start()` 本地报错
    （`1001`），不浪费连接。
  - 示例 `examples/v3_realtime_asr` 新增 `-speaker-context` /
    `-speaker-context-id`；中英文 README 新增
...
- **录音文件识别（v2）词级时间戳恒为 0**：`DescribeTaskStatus` 返回的
  ...
  `SentenceWords.OffsetStartMs` / `OffsetEndMs` 全部为 0。现在通过自定义
  `UnmarshalJSON` 优先读 `StartTime` / `EndTime`，并保留
  `OffsetStartMs` / `OffsetEndMs` 作为回退兼容（显式 0 不会被回退覆盖）。
...
- README 鉴权章节按**在线（流式）/ 离线（HTTP）**拆分，各自给出 `auth` 字段表，
  ...
  传入固定签名后不再刷新、签名与站点（`credential.SetSite(common.SiteIntl)`）绑定。
  中英文 README 同步。
